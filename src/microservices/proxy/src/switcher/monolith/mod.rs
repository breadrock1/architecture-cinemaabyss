...
impl Monolith {
    pub fn new(url: &str) -> Monolith {
        Monolith { url: url.to_owned() }
    }
}

impl APIProvider for Monolith {}

#[async_trait::async_trait]
impl HealthProvider for Monolith {
    async fn health(&self) -> anyhow::Result<()> {
        let url = format!("{}{}", self.url, HEALTH_URL);
...
impl MovieProvider for Monolith {
    ...
    async fn get_movie(&self, movie_id: i32) -> anyhow::Result<Movie> {
        let url = format!("{}{}/{}", &self.url, MOVIES_URL, movie_id);
...
#[async_trait::async_trait]
impl UserProvider for Monolith {
    ...
    async fn get_user(&self, user_id: i32) -> anyhow::Result<User> {
        let url = format!("{}{}/{}", &self.url, USERS_URL, user_id);
...
#[async_trait::async_trait]
impl PaymentProvider for Monolith {
    ...
    async fn get_payment(&self, payment_id: i32) -> anyhow::Result<Payment> {
        let url = format!("{}{}/{}", &self.url, PAYMENTS_URL, payment_id);
...
impl SubscriptionProvider for Monolith {
    ...
    async fn get_subscription(&self, subscription_id: i32) -> anyhow::Result<Subscription> {
        let url = format!("{}{}/{}", &self.url, SUBSCRIPTIONS_URL, subscription_id);
